...
const clearSteps = document.querySelector("#clearSteps");
...
function setupEvents() {
  ...
  stepsForm.addEventListener("submit", (event) => {
    event.preventDefault();
    normalizeNumericField(stepsInput, false);
    updateEntry(selectedDate, { steps: readNumber(stepsInput.value) }, "Steps saved");
  });

  document.querySelectorAll("[data-steps-add]").forEach((button) => {
    button.addEventListener("click", () => {
      addToEntry(selectedDate, "steps", readNumber(button.dataset.stepsAdd), "Steps updated");
    });
  });

  clearSteps.addEventListener("click", () => {
    updateEntry(selectedDate, { steps: 0 }, "Steps cleared");
  });
...
function renderCalendar() {
  ...
  for (let day = 1; day <= lastDate; day += 1) {
    ...
    if (entry.steps > 0) {
      markers.appendChild(createMarker("steps", `Steps: ${formatNumber(entry.steps)}`));
    }
    if (entry.water > 0) {
...
function formatTooltip(entry) {
  const parts = [
    ...
    `${formatNumber(entry.steps)} steps`,
